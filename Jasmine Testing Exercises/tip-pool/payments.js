...
function createCurPayment() {
  ...
  if (Number(billAmt) > 0 && Number(tipAmt) >= 0) {
    return {
      billAmt: billAmt,
      tipAmt: tipAmt,
      tipPercent: calculateTipPercent(billAmt, tipAmt), //calculateTipPercent() in helpers.js!
    };
  }
}
...
function appendPaymentTable(curPayment) {
  ...
  appendTd(newTr, "$" + curPayment.billAmt); //appendTd() in helpers.js!
...
function updateSummary() {
  let tipPercentAvg;
  let paymentTotal = sumPaymentTotal("tipPercent"); //sumPaymentTotal() in helpers.js!
