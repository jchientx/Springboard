...
SELECT * FROM analytics WHERE last_updated = (SELECT MIN(last_updated) FROM analytics) LIMIT 1;
...
-- FS2. Find all the apps that have a name similar to “facebook”.  ** ILIKE
...
-- FS4. Find all the apps that have education as one of their genres. @> is the "contains" operator. <@  is contained by
